...
\noindent donde se ha de negar la señal cuadrada empleada en el muestreo natural alli presente. Matemáticamente, siendo la cuadrada de altura unitaria, esto se puede modelar bajo la relación:

\begin{align*}
    \overline{m}(t) = 1 - m(t)
\end{align*}

\noindent Espectralmente esto se corresponde con:

\begin{align*}
    \overline{M}(f) = \delta(f) - M(f)
\end{align*}

\noindent por lo que al multiplicar la entrada temporalmente con esta señal, espectralemente resulta en el espectro de la señal menos el espectro del muestreo natural previamente analizado:
...
Dadas las señales con $f_{in} = 15kHz$:

\begin{itemize}
    \item $x_3(t) = A \cdot \cos(2\pi \cdot f_{in}\cdot t)$    \item $x_4(t) = \text{onda cuadrada, con } T=2/f_{in} \ \& \ DC=75\%$
\end{itemize}

Se piden analizar diferentes configuraciones:
